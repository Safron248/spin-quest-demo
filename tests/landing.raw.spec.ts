/**
 * Legacy artifact, kept for demo purposes only.
 *
 * Shows the old style where each test creates its own `new LandingPage(page)`,
 * before the PageObjectManager and the `pom` fixture removed that boilerplate.
 * The current version lives in `landing.spec.ts`.
 */
...
test.describe('Landing page (manual page-object instances)', () => {
  // boilerplate: declare it here, assign it in beforeEach, hope nothing reads it
...
// a test outside the describe block has to build its own instance from scratch
test('open site (standalone test, its own instance)', async ({ page }) => {
